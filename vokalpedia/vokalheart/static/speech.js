...
let csrfToken = {'csrfToken': "" }
  ...
  const token = () => {
    ...
    .then(data => {
      csrfToken.csrfToken = data.csrfToken; // Stockage du token dans un objet
      // Utiliser le token CSRF dans les requêtes POST, PUT, PATCH ou DELETE
    })
    .catch(error => console.error('Erreur:', error));
  };
  
  const token_recup = token().then(() => {
    // L'appel à postDataToDjango() ne se fait que lorsque le token est récupéré
    postDataToDjango();
  });
...
const postDataToDjango = () => {
    fetch('/wikispeech', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRFToken': csrfToken.csrfToken  // Utilisation du token CSRF récupéré
        },
